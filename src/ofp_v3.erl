...
encode_body(#ofp_flow_mod{cookie = Cookie, cookie_mask = CookieMask,
    ...
    <<Cookie:8/bytes, CookieMask:8/bytes,
      TableInt:8, CommandInt:8, Idle:16, Hard:16,
      Priority:16, Buffer:32, OutPortInt:32,
      OutGroupInt:32, FlagsBin:2/bytes, 0:16,
      MatchBin/bytes, InstructionsBin/bytes>>;
...
decode_body(flow_mod, Binary) ->
    <<Cookie:8/bytes, CookieMask:8/bytes, TableInt:8,
      CommandInt:8, Idle:16, Hard:16,
      Priority:16, Buffer:32, OutPortInt:32,
      OutGroupInt:32, FlagsBin:2/bytes, 0:16,
      Data/bytes>> = Binary,
    Table = ofp_v3_map:decode_table_id(TableInt),
    ...
    <<_:16, MatchLength:16, _/bytes>> = Data,
    MatchLengthPad = MatchLength + (8 - (MatchLength rem 8)),
    InstrLength = size(Binary) - ?FLOW_MOD_SIZE + ?OFP_HEADER_SIZE + ?MATCH_SIZE - MatchLengthPad,
